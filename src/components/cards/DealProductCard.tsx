...
export default function DealProductCard({
  ...
  return (
    <div
      ...
      className="group flex flex-col h-full bg-paper-soft relative border border-line transition-all duration-500 hover:border-accent hover:shadow-lg"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* Deal banner — High-contrast luxury deal banner */}
      <div className="flex items-center justify-between bg-panel text-ink border-b border-line px-2.5 sm:px-3.5 py-1.5 sm:py-2.5">
        <span className="font-mono text-[9px] sm:text-xs font-black tracking-wider bg-accent text-white px-1.5 sm:px-2 py-0.5">-{discountPct}% OFF</span>
        <span className="font-mono text-[8px] sm:text-xs font-bold text-muted tracking-wide">
          AHORRO S/. {savingsAmount.toFixed(0)}
        </span>
      </div>

      {/* Product Photo Stage */}
      <div className="relative aspect-[3/4] w-full overflow-hidden bg-panel select-none border-b border-line flex flex-col justify-between p-2 sm:p-4 text-ink transition-all duration-500">
        {activeImage && (
          <img
            src={activeImage}
        ...
        <div className="hidden sm:flex justify-between items-start w-full z-10 text-[9px] font-mono text-zinc-200 uppercase tracking-wide drop-shadow">
        ...
        <div className="hidden sm:block w-full z-10 space-y-1.5 pt-2 border-t border-white/15">
          <div className="flex justify-between items-center text-[9px] font-mono text-zinc-200 uppercase drop-shadow">
        ...
        {product.stock <= 4 && product.stock > 0 && (
          <span className="absolute top-2 left-2 sm:top-9 sm:left-3 z-10 flex items-center gap-1 text-[7px] sm:text-[8px] font-mono font-bold uppercase tracking-wider px-1.5 py-0.5 bg-accent text-white shadow-md">
        ...
        <button
          ...
          className={`absolute top-2 right-2 sm:top-9 sm:right-3 p-1.5 rounded-none backdrop-blur-md transition-all duration-300 z-10 ${
            isFavorite
              ? 'bg-accent text-white border border-rose-600 shadow-[0_0_10px_rgba(225,29,72,0.4)]'
        ...
        <div
          className={`absolute inset-x-0 bottom-0 bg-paper-soft/95 backdrop-blur-md p-3 transition-all duration-300 hidden sm:flex flex-col gap-2 border-t border-line z-20 text-ink ${
      ...
      <div className="flex-1 flex flex-col font-sans p-2.5 sm:p-4">
        ...
        <h3 className="font-sans font-bold text-[11px] sm:text-sm text-ink tracking-wide leading-tight mb-1 group-hover:text-accent transition-colors line-clamp-1">
          {product.name}
        </h3>

        <p className="text-[8.5px] sm:text-[10px] text-muted mb-1 mt-auto font-mono truncate">
